...
<script src="/_dir/jquery-1.6.4.min.js"></script>
...
function GetSql($md5path)
{
	$filelist = file_get_contents("/var/www/html/_dir/patchreadme/$md5path/patch_tmp/files/update.sql");
	return $filelist;
}
...
foreach($md5pathA as $md5path)
{
	...
	$f["sql"] = GetSql($md5path);
	
	
	if(strstr($f["sql"], "alter table") 
	||  strstr($f["sql"], "call proc_EditTable") 
	||  strstr($f["sql"], "call CREATE TABLE") 
	)
	{
		$allf[] = $f;
	}
...
<?php
		...
		foreach($allf  as $f)
		{
			...
			echo "<td>{$f['custom']}</td>";
			echo "<td  class=patchfile fname=$id><textarea>{$f['sql']}</textarea></td>";
			echo "</tr>";
			$n++;
		}
