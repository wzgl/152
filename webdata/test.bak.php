...
$packetsList = getUploadPatchNameList($logcontent);

var_dump($packetsList);
    ...
    foreach($files as $file)
    {
        ...
        if(file_exists($file["path"]))
        {
            foreach($split_filter as $key)
            {
                ...
                if(false !== mb_stripos($file["name"],$key,0,"gbk") ||
                    false !== mb_stripos($file["path"],$key, 0, "gbk") ||
                    false !== mb_stripos($file["user"],$key, 0, "gbk") ||
                    false !== mb_stripos($file["etime"],$key, 0, "gbk") ||
                    false !== mb_stripos($file["size2"],$key, 0, "gbk"))
                {
                    $all_files[] = $file;
                    break;
                }

                //add by baiyf 2016-10-12 支持对文件列表的搜索，查看点击详情查看
                ...
                if($packet_file_list && count($packet_file_list)>0){
                    ...
                    foreach($fs as $one_file){
                        ...
                        if(false !== mb_stripos($one_file,$key,0,"gbk"))
                        {
                            $all_files[] = $file;
                            break;
                        }
...
$bIsAsc = 1;
{
    ...
    if(isset($_REQUEST['D']))
        $bIsAsc = $_REQUEST['D'];
    fileSort($all_files,$keyName,$bIsAsc);
    $bIsAsc = $bIsAsc?0:1;
}
...
<table>
    <tr>
        ...
        <th class="td2"><a href="<?php echo "?C=link&D=$bIsAsc&search=$filters";?>">Name</a></th>
        <th width="200px"><a href="<?php echo "?C=etime&D=$bIsAsc&search=$filters";?>">Last Modified</a></th>
        <th width="100px"><a href="<?php echo "?C=size&D=$bIsAsc&search=$filters";?>">Size</a></th>
        <th width="70px"><a href="<?php echo "?C=user&D=$bIsAsc&search=$filters";?>">User</a></th>
