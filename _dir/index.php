...
<div style="float:right">
    <a href="###" onclick="showallfile(true)" title="显示所有冲突文件">
        <img width="40px" style="border:0px;" src="/conflict.png"/></a>
...
<div style="float:right;width:460;height: 45px;margin-right:10px;margin-bottom: 2px;">
    <?php if (stripos($dirstart . $dir, "del_patch") === false) { ?>
        <form action="/webdata/search_file.php" method="post" target="_blank">
            <input type="text" value="" name="search" id="input_search"/>
            <select name="isdetailed" id="select_search">
                <option value="0">包</option>
                <option value="1">修改文件</option>
            </select>
...
<table>
    ...
    foreach ($fs as $f) {
        ...
        if (!$f["isdir"] && strstr($f["name"], ".exe.rar")) {
            ...
            if (stripos(($dirstart . $dir), "del_patch") === false) {
            ...
            $ext_name = pathinfo($f["name"], PATHINFO_EXTENSION);
    ...
    <form id="delfile" target="deliframe" method="post" action="/webdata/delasmpatch.php">
        ...
        <center>
            <table>
                ...
                <tr>
                    <td>&nbsp;</td>
                    <td><input id="delsubmit" style="width:200px" name="delsubmit" type="submit"
                               value="&nbsp;&nbsp;删&nbsp;&nbsp;除&nbsp;&nbsp;"/></td>
                </tr>
            </table>
        </center>
...
<div id="upload_dlg"
    ...
    <form id="upfileform" target="upiframe" method="post" action="/webdata/uploadasmpatch.php"
        ...
        <center>
            <table>
                <tr>
                    <td><label>支持格式:</label></td>
                    <td><label>.pdf .rar .zip .xls .xlsx .doc .docx </br>.ppt .pptx .exe .png .jpeg .gif .jpg</label>
                    </td>
                </tr>
                <tr>
...
<div style="display:none">
    <form method="post" target="_blank" action="/webdata/showallfilelist.php">
        ...
        <input id="hidefile" name="hidefile" type="text" value=""/>
